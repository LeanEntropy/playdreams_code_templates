...
## and turret-following camera. Two cameras: TankCamera (3rd person) and
## IsometricCamera (fixed-angle overhead). Press C to switch.
...
func _ready() -> void:
	...
	get_tree().root.add_child.call_deferred(_iso_camera)
...
func _load_config_overrides() -> void:
	forward_speed = GameConfig.get_value("tank", "forward_speed", forward_speed)
	reverse_speed = GameConfig.get_value("tank", "reverse_speed", reverse_speed)
	hull_turn_speed = GameConfig.get_value("tank", "hull_turn_speed", hull_turn_speed)
	acceleration = GameConfig.get_value("tank", "acceleration", acceleration)
	deceleration = GameConfig.get_value("tank", "deceleration", deceleration)
	mouse_sensitivity = GameConfig.get_value("tank", "mouse_sensitivity", mouse_sensitivity)
	turret_speed_limited = GameConfig.get_value("tank", "turret_speed_limited", turret_speed_limited)
	turret_rotation_speed = GameConfig.get_value("tank", "turret_rotation_speed", turret_rotation_speed)
...
func _physics_process(delta: float) -> void:
	...
	if terrain_follow_enabled and is_on_floor():
		...
		rotation = rotation.lerp(Vector3(0, current_y, 0), 5.0 * delta)
...
func _update_iso_camera() -> void:
	if not _iso_camera or not is_instance_valid(_iso_camera) or not _iso_camera.is_inside_tree():
		return
	_iso_camera.global_position = global_position + Vector3(iso_distance, iso_height, iso_distance)
...
func _apply_terrain_rotation(delta: float, preserve_y: float) -> void:
	...
	for corner_local in corners:
		...
		var query := PhysicsRayQueryParameters3D.create(
			corner_world + Vector3(0, 0.5, 0),
			corner_world - Vector3(0, terrain_raycast_length, 0)
		)
		query.exclude = [self]
