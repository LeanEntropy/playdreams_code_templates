...
func initialize(player_node: CharacterBody3D) -> void:
	Log.info("FirstPersonController initializing...")
	
	# Validate player node
	if player_node == null:
		Log.error("FirstPersonController: Player node is null!")
		return
	...
	if player_camera_rig:
		...
		if spring_arm:
			...
			if camera:
				...
				Log.info("FirstPersonController: PlayerCamera activated")

	if not camera:
		Log.error("FirstPersonController: Failed to find PlayerCamera!")
		return
	...
	Log.info("FirstPersonController: Player mesh hidden (FPS standard)")
	Log.info("FirstPersonController initialized successfully")
...
func cleanup() -> void:
	...
	is_active = false
	Log.info("FirstPersonController cleaned up")
